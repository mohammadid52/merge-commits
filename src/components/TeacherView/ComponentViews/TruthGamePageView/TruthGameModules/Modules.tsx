...
const Modules = (props: ModulesProps) => {
  ...
  return (
    <div
      className={`${
        fullscreen ? 'md:mb-0 px-4 md:px-8 py-4' : 'p-2'
      } md:h-5.8/10 w-full bg-gradient-to-tl from-dark-blue to-med-dark-blue text-gray-200 rounded-lg overflow-hidden`}>
      ...
      <div className={`${fullscreen ? 'h-full' : 'h-8.5/10'} w-full h-full `}>
        {formInputs
          ? inputs.map((input, key) => (
              <div
                key={key}
                className={`flex flex-col animate-fadeIn pb-4 ${
                  key !== inputs.length - 1 && 'border-b border-white border-opacity-10 '
                }`}>
                <label
                  className={`${
                    fullscreen ? 'text-sm md:text-md mb-2' : 'text-xs'
                  } font-light text-blue-100 text-opacity-70`}
                  htmlFor={input.name}>
                  {input.prompt}
                </label>
                <input
                  id={input.name}
                  className={`${
                    fullscreen ? 'text-sm md:text-lg md:w-72 ' : 'text-xs w-9/10'
                  } px-4 py-1 rounded-lg text-gray-700 bg-gray-300`}
                  name={input.name}
                  type="text"
    ...
    //     <h3 className="text-xl  font-bold mb-3 border-b-0 border-white mb-2">Focus Questions</h3>
