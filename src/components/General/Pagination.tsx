            ...
            <div className="flex-1 flex items-center justify-end">
                ...
                <div className="w-3/10">
                    <nav className="relative z-0 inline-flex shadow-sm font-bold rounded-xl">
                        <div onClick={backward} className={`${currentPage === 1 ? 'text-gray-800' : 'cursor-pointer text-gray-300 hover:text-gray-400'}  font-bold relative inline-flex items-center px-2 py-2 rounded-l-md  text-sm leading-5 font-medium  focus:z-10 focus:outline-none focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150`} aria-label="Previous">
                        ...
                        {pageNumbers.map(number => (
                            <div key={number} onClick={() => paginate(number)} className={`${number === currentPage ? 'text-white rounded-full font-bold' : 'text-gray-700 cursor-pointer hover:text-gray-400' } font-semibold flex justify-center -ml-px relative items-center px-4 py-2  text-sm leading-5 font-medium  focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150`}>
                                {number}
                            </div>
                        ))}
                        <div onClick={forward} className={`${currentPage === data.length ? 'text-gray-800' : 'cursor-pointer text-gray-300 hover:text-gray-400' } font-bold -ml-px relative inline-flex items-center px-2 py-2 rounded-r-md   text-sm leading-5 font-medium focus:z-10 focus:outline-none focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150`} aria-label="Next">
                            <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
