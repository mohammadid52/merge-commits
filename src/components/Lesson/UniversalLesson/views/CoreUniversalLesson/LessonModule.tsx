...
import {
  Tabs3,
  useTabs,
} from "../../../UniversalLessonBuilder/UI/UIComponents/Tabs/Tabs";

export const getFilteredUniversalSyllabus = (
  curriculum: any,
  lessonId: string
) => {
  ...
  curriculums.forEach((curriculum: any) => {
    ...
    isCourseAdded &&
      selectedCurriculums.push({
        ...curriculum,
        assignedSyllabi: assignedSyllabi.map(
          (syllabus: any) => syllabus.unit.name
        ),
...
const EvidenceTab = ({
  ...
  const fetchObjectives = async (curricularId: string) => {
    ...
    const [results, learningObjectiveSeqResult, topics]: any =
      await Promise.all([
        await API.graphql(
  ...
  const fetchCurriculum = async () => {
    try {
      ...
      let selectedCurriculums = getSelectedCurriculum(
        curriculums,
        currentLesson.id
      );
      ...
      curriculum.learningObjectiveData.forEach(
        (objective: {
          ...
          if (
            Boolean(
              curricula.find(
                (c: { id: string }) => c.id === objective.curriculumID
              )
            )
          ) {
        ...
        >
          {checkedEvidence.map((evidence: any) => (
            ...
            >
              ...
              <p className="text-gray-900 dark:text-white  w-auto">
                {evidence.name}
              </p>
            </div>
          ))}
