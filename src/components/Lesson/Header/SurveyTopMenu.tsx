...
const SurveyTopMenu = (props: LessonHeaderBarProps) => {
  ...
  useEffect(() => {
    if (Object.keys(state.questionData).length > 0 && nrQuestions === 0) {
      const totalQuestionNr = Object.keys(state.questionData).reduce(
  ...
  useEffect(() => {
    ...
    if (Object.keys(state.questionData).length > 0) {
      ...
      const roundedPercentage = Math.round(
        (getTotalQuestionAnsweredNumber() / nrQuestions) * 100
      );
  ...
  const getTotalQuestionAnsweredNumber = (): number => {
    return Object.keys(state.questionData).reduce(
    ...
    <div
      ...
      <div className={`h-8 max-w-256 flex flex-col justify-between`}>
        ...
        <div className="overflow-hidden h-2 text-xs flex rounded-full bg-medium-gray">
          <div
            style={{width: `${completion}%`, transition: 'width 0.5s linear'}}
            className="transition rounded-full duration-500 ease-in-out shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-sea-green"></div>
        </div>
      </div>
    </div>
  );
