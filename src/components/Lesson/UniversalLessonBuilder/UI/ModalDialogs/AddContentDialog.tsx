...
import YouTubeMediaDialog from './YouTubeMediaDialog';
import ModalPopIn from '../../../../Molecules/ModalPopIn';

interface IAddContentInterface {
...
const AddContentDialog = ({createNewBlockULBHandler}: IAddContentInterface) => {
  const [isVideoDialogOpen, setIsVideoDialogOpen] = useState<boolean>(false);
  const onYouTubeVideoBtnToggle = () => {
    setIsVideoDialogOpen((prevValue) => !prevValue);
  };
  ...
  return (
    <>
      {isVideoDialogOpen ? (
        <ModalPopIn
          closeAction={onYouTubeVideoBtnToggle}
          inputJSX={<YouTubeMediaDialog />}
          saveAction={() => createNewBlockULBHandler("","")}
        />
      ) : (
        <div className={` z-50 `}>
          <div className="relative flex items-center">
            <h2 className="w-auto bg-white text-lg font-medium text-gray-900">
              Add Content
            </h2>
          </div>
                ...
                <div className={`grid grid-cols-1 gap-2 h-auto w-full`}>
                  <div
                    ...
                    <IconContext.Provider
                      value={{size: '64px', className: 'text-gray-800'}}>
                      <MdTitle />
                    </IconContext.Provider>
                  </div>
            ...
            <div>
              ...
              <div className={`h-full w-full`}>
                <div className={`grid grid-cols-1 gap-2 h-auto w-full`}>
                  ...
                  <div
                    className={`w-full h-24 flex flex-col justify-center items-center bg-gray-200 rounded cursor-pointer`}
                    onClick={onYouTubeVideoBtnToggle}>
                    <IconContext.Provider
                      value={{size: '64px', className: 'text-gray-800'}}>
                      <AiOutlineYoutube />
                    </IconContext.Provider>
