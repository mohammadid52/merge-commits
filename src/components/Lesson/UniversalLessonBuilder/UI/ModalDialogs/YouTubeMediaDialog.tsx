...
const YouTubeMediaDialog = ({
  ...
  const onSave = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isValid) {
      ...
      if (isEditingMode) {
        ...
        const updatedList: any = createNewBlockULBHandler("", "", "video", [
          input,
        ]);
        await addToDB(updatedList);
      }
  ...
  const checkUrl = () => {
    if (!videoInputs.value) {
      setError(
        UniversalBuilderDict[userLanguage]["FORMS_ERROR_MSG"]["VIDEO_REQUIRED"]
      );
      return false;
    }
    if (!youTubeVideoRegex.test(videoInputs.value)) {
      setError(
        UniversalBuilderDict[userLanguage]["FORMS_ERROR_MSG"]["VIDEO_INVALID"]
      );
      return false;
    } else {
      setError("");
        ...
        <div className={`grid grid-cols-3 gap-2`}>
          <div className={`col-span-2`}>
            <FormInput
              ...
              name="value"
              label={
                UniversalBuilderDict[userLanguage]["FORMS"]["VIDEO_URL_LABEL"]
              }
          ...
          <div>
            <Selector
              ...
              label={
                UniversalBuilderDict[userLanguage]["FORMS"]["VIDEO_SIZE_LABEL"]
              }
              selectedItem={size || ""}
            />
          </div>
