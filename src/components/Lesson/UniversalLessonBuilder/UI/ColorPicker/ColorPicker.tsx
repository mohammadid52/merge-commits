            ...
            {dynamicValues.map((code: ColorObject, idx2: number) => {
              return (
                ...
                >
                  {classString
                    ?.split(" ")
                    .indexOf(`bg-${color.value}-${code.value}`) > -1 ||
                  classString
                    ?.split(" ")
                    .indexOf(`border-${color.value}-${code.value}`) > -1 ? (
                    <div
    ...
    >
      ...
      <div className={`bg-white shadow-lg  my-3 rounded-lg p-6`}>
        <div className="flex items-center mb-2 justify-between">
          <p className={`text-gray-900 w-auto font-medium text-2xl`}>
            Select a color
          </p>
          {noneLabel && (
            <Buttons
        ...
        <div className={`my-4 ${customColors ? "flex items-center" : ""}`}>
          {dynamicColors.length > 0 && dynamicValues.length > 0
            ? colorGrid()
            : null}
        </div>
      </div>
