...
const Modules = (props: ModulesProps) => {
  ...
  useEffect(() => {
    ...
    if (
      cookies[`lesson-${state.syllabusLessonID}`]?.truthGame?.additional &&
      cookies[`lesson-${state.syllabusLessonID}`].truthGame.additional.length > 0
    ) {
      cookies[`lesson-${state.syllabusLessonID}`].truthGame.additional.forEach(
    ...
    if (
      ...
      state.componentState.truthGame.additional &&
      state.componentState.truthGame.additional.length > 0
    ) {
      state.componentState.truthGame.additional.map(
        (item: {name: string; input: string}) => {
          setFormInputs((prev) => {
            return {
              ...prev,
  ...
  useEffect(() => {
    if (
      ...
      setCookie(`lesson-${state.syllabusLessonID}`, {
        ...
        truthGame: {
          ...cookies[`lesson-${state.syllabusLessonID}`].truthGame,
          additional: tempArray,
        },
      });
    }
  }, [formInputs]);
        ...
        {formInputs
          ? inputs.map((input, key) => (
              <div
                ...
                <label
                  className="text-sm md:text-md mb-2 font-light text-base text-blue-100 text-opacity-70"
                  htmlFor={input.name}>
                  {input.prompt}
                </label>
                <input
                  id={input.name}
                  className="px-4 py-1 text-lg rounded-lg text-gray-700 bg-gray-300"
                  name={input.name}
                  type="text"
