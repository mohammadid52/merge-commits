  ...
  return (
    <div className={`flex justify-between text-sm md:text-base text-gray-200 mb-4 md:mb-0`}>
      {dataProps ? (
          ...
          return (
            <div key={key} className={`font-open font-light h-auto rounded-lg py-2`}>
              ...
              <div className="w-full overflow-y-auto overflow-x-hidden">
                {item.input
                  ? wordArray.map((word: string, key: number) => <p key={`poll_module_${key}`}>{word}</p>)
                  : null}
              </div>
            </div>
          );
        ...
        <div className={`font-open font-light h-auto rounded-lg py-2`}>
          <div className="w-full overflow-y-auto overflow-x-hidden">No poll answers...</div>
        </div>
      )}
    </div>
  );
