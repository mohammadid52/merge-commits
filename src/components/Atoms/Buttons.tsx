...
const Buttons: React.FC<ButtonProps> = (btnPrps: ButtonProps) => {
  ...
  return (
    <button
      ...
      className={`font-bold uppercase text-xs px-4 py-2 rounded-lg flex items-center justify-center w-auto ${theme.outlineNone} ${transparent ? theme.btn.cancel : theme.btn[themeColor]} ${btnClass ? btnClass : ''} ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
      onClick={onClick}
    >
