    ...
    <div className="relative" ref={currentRef} onFocus={() => onFocus()}>
      <span className="inline-block w-full h-full rounded-md shadow-sm">
        <button
          ...
          <span className="block truncate text-gray-700">{selectedItem?.value ? selectedItem.value : placeholder}</span>
          <span
            ...
            {teacherList.length > 0 ? (
              teacherList.map((item: { name: string; id: any; value: string; avatar?: string }, key: number) => (
                <li
                  ...
                  <span
                    className={`${selectedItem.id === item.id ? 'font-semibold' : 'font-normal'} pl-4 block truncate`}>
                    {item.name}
                  </span>
                  <span
