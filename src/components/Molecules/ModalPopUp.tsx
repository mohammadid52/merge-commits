...
const ModalPopUp = (props: ModalProps) => {
  const { saveAction, closeAction, message, saveLabel, onlyInfo, deleteModal, deleteLabel, cancelLabel } = props;
  return (
    <Modal showHeader={true} showHeaderBorder={false} showFooter={false} closeAction={closeAction}>
      <div className="w-168 text-center my-8">
        ...
        <div className="flex justify-around mt-16 w-5/10 mx-auto">
          {!onlyInfo &&
            <Buttons label={cancelLabel ? cancelLabel : "Cancel"} btnClass='px-8 py-3 mr-4' onClick={closeAction} transparent />
          }
