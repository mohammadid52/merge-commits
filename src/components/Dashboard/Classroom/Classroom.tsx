...
const Classroom: React.FC<ClassroomProps> = (props: ClassroomProps) => {
  ...
  // ############################ BASIC STATE ############################ //
  // ##################################################################### //
  const { state, dispatch, theme, userLanguage, clientKey } =
    useGlobalContext();
  ...
  useEffect(() => {
    if (!isEmpty(roomId) && state.roomData?.rooms?.length > 0) {
      const roomIndex = state.roomData.rooms.findIndex(
        (d: any) => d.id === roomId
      );
  ...
  useEffect(() => {
    ...
    const activeSyllabusLessons =
      syllabusList.find(
        (syllabus: any) => syllabus.id === activeRoomInfo?.activeSyllabus
      )?.lessons?.items || [];
  ...
  const handleLessonMutationRating = async (
    lessonID: string,
    ratingValue: string
  ) => {
    if (!fetchingPersonData) {
      ...
      try {
        ...
        logError(
          error,
          { authId, email },
          "Classroom @handleLessonMutationRating"
        );
      }
    }
  };
        ...
        <div className="relative px-5 2xl:px-0 lg:mx-auto lg:max-w-256 md:max-w-none">
          ...
          <div>
            {isTeacher && teachingStyle === TeachingStyle.PERFORMER && (
              <Warning
                message={"This classroom has Performer mode activated"}
              />
            )}
            {isTeacher && (
              <>
            ...
            <>
              <SectionTitleV3
                ...
                title={`${
                  ...
                  state.roomData?.activeSyllabus || "loading"
                }`}
                subtitle={
                  isTeacher
