...
const MainSummary = (props: LessonCardProps) => {
  ...
  return (
    <div className={`${lessonType !== 'survey' ? 'h-44' : 'h-auto'} p-4 px-5 flex flex-col justify-start items-center`}>
      ...
      {lessonType === 'survey' ? (
        <p
          ...
          dangerouslySetInnerHTML={{
            __html: lessonProps.lesson && lessonProps.lesson?.purpose ? lessonProps.lesson?.purpose : '',
          }}></p>
      ) : (
        <p
          className="overflow-ellipsis overflow-hidden ... text-md mt-1 text-gray-500 text-left"
          dangerouslySetInnerHTML={{
            __html: lessonProps.lesson && lessonProps.lesson?.summary ? lessonProps.lesson?.summary : '',
          }}></p>
      )}
    </div>
  );
