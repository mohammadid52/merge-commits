...
const StandardLessonCard = (props: LessonCardProps) => {
  const { keyProps, lessonProps, accessible } = props;
  const { theme } = useContext(GlobalContext);

  return (
      <div key={keyProps} className={`relative bg-white rounded border border-dark-gray border-opacity-10 h-auto flex mb-8 ${theme.elem.text} `}>
        <div
        ...
        <div className="w-7.5/10 flex flex-col ">
          <div className="h-44 p-4 flex flex-col justify-start items-center">
            <h1 className={`text-2xl text-black font-open text-left`}>
              {lessonProps && lessonProps.lesson.title ? lessonProps.lesson.title : null}
            </h1>
            <p className="text-sm text-left">
              {lessonProps && lessonProps.lesson.summary ? lessonProps.lesson.summary : null}
            </p>
          </div>
          <div className={`h-2/10 ${theme.dashboard.bg} flex justify-between text-sm  rounded-br`}>
