...
const Start: React.FC<StartProps> = ({
  ...
  const isTeacher =
    teachingStyle !== TeachingStyle.PERFORMER
      ...
      : false;
  const isOnDemand =
    teachingStyle === TeachingStyle.PERFORMER ? true : user.onDemand;
  ...
  const fetchAttendance = async () => {
    try {
      ...
      if (syllabusData) {
        ...
        if (isMounted) {
          setAttendanceRecorded(
            Boolean(list?.data.listAttendances?.items.length)
          );
        }
      }
    } catch (error) {
  ...
  const setSyllabusLessonHistory = async (
    syllabusID: string,
    historyArray: any[]
  ) => {
    try {
