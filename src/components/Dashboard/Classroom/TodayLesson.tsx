...
const Today: React.FC<ClassProps> = (props: ClassProps) => {
  ...
  return (
    <div
      className={`relative bg-white rounded-xl shadow-container ${theme.elem.text}  w-full h-auto flex mb-16`}>
      <div className={`w-2.5/10 bg-dark rounded-tl-xl rounded-bl-xl`}>
        <div className='h-6/10 flex justify-center items-center'>
          <img
            className=' w-32 rounded-full '
            src={`${curriculum && curriculum.artist.images ? curriculum.artist.images : null}`}
            alt={`${curriculum && curriculum.artist.name ? curriculum.artist.name : ''}`}
          />
        </div>
        ...
        <div className='h-3/10 flex flex-row-reverse'>
          <h2
            className={`first w-6/10 text-4xl font-open leading-8 font-medium tracking-widest mb-4 text-gray-200`}>
            <p>{curriculum && curriculum.artist.name ? curriculum.artist.name : null}</p>
          </h2>
        </div>
      </div>
      <div className='relative w-7.5/10 flex flex-col '>
        <div className='absolute w-auto top-0 right-0'>
          <ToolTip
            position='bottom-left'
            color='#black'
            header='Todays lesson'
