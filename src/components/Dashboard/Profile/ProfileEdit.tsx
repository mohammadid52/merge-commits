...
const ProfileEdit = (props: UserInfoProps) => {
  ...
  const { user, getUser, status, setStatus, stdCheckpoints, questionData } =
    props;
  ...
  const onMultipleSelection = (
    ...
    if (selectedQuestion?.length > 0) {
      ...
      const selectedOption: any = selectedQuestion?.find(
        (item: any) => item.id === id
      );
      let updatedList;
      if (selectedOption) {
  ...
  if (imagePreview) {
    imagePreview = <img src={`"${user.image}"`} />;
  } else {
    imagePreview = (
      <svg
                  ...
                  <>
                    ...
                    <div className="sm:col-span-3 p-2">
                      <Selector
                        ...
                        selectedItem={
                          editUser.language === "EN" ||
                          editUser.language === "English"
                            ? "English"
                            : "Spanish"
                        }
                      />
