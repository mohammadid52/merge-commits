...
const UserLookup = () => {
  ...
  async function listUsers(
    ...
    try {
      ...
      if (!isPrev && !isTokenStored) {
        ...
        currentPage === 1 ? setFirstPage(true) : setCurrentPage(currentPage - 1);
      }
  ...
  useEffect(() => {
    const newUserList = [...userList].sort((a, b) =>
      ...
      <div className={`w-full h-full mt-4`}>
        ...
        <div className="flex justify-between">
          ...
          <div className="flex justify-end py-4 mb-4">
            ...
            />
            <Selector
              placeholder={UserLookupWithTokenDict[userLanguage]['sortby']}
              list={sortByList}
            ...
            />
            <button
              className={`w-28 bg-gray-100 mr-4 p-3 border-gray-400  border-0 rounded ${
                theme.outlineNone
              } ${
                sortingType.asc
                  ? 'border-l-0 rounded-l-none'
                  : ' border-r-0  rounded-r-none transform rotate-180'
              }`}
            ...
            <Buttons
              label={UserLookupWithTokenDict[userLanguage]['button']['add']}
              onClick={handleLink}
                ...
                <div className="w-full flex justify-between border-b-0 border-gray-200 ">
                  ...
                  <div className="w-1/10 flex justify-center px-8 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                    <span className="w-auto">
                      {UserLookupWithTokenDict[userLanguage]['role']}
                    </span>
                  </div>
                  <div className="w-3.5/10 flex justify-center px-8 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                    <span className="w-auto">
                      {UserLookupWithTokenDict[userLanguage]['institution']}
                    </span>
                  </div>
                  <div className="w-1.5/10 flex justify-center px-8 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                    <span className="w-auto">
                      {UserLookupWithTokenDict[userLanguage]['status']}
                    </span>
                  </div>
