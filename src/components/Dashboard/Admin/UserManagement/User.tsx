        ...
        <div className={`w-full white_back p-8 ${theme.elem.bg} ${theme.elem.text} ${theme.elem.shadow} mb-8`}>
          <div className="h-9/10 flex flex-col md:flex-row">
            <div className="w-auto p-4 flex flex-col text-center items-center">
              {user.image ? (
                <img src={imageUrl} className="w-20 h-20 md:w-40 md:h-40 rounded-full flex flex-shrink-0" />
              ) : (
                <div
            ...
            <Switch>
              <Route
                path={`${match.url}/edit`}
                render={() => <UserEdit user={user} status={status} setStatus={setStatus} getUserById={getUserById} />}
              />
