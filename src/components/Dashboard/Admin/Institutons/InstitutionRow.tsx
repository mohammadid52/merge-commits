...
const InstitutionRow: React.FC<InstitutionRowProps> = (instRowPrps: InstitutionRowProps) => {
    ...
    <div id={instRowPrps.id} className="flex justify-between bg-white w-full border-b-0 border-gray-200">
              ...
              <div
                ...
                style={{
                  background: `${stringToHslColor(
                    getInitialsFromString(instRowPrps.name)[0] + ' ' + getInitialsFromString(instRowPrps.name)[1]
                  )}`,
                  textShadow: '0.1rem 0.1rem 2px #423939b3',
                }}>
                {instRowPrps.name
                  ? initials(getInitialsFromString(instRowPrps.name)[0], getInitialsFromString(instRowPrps.name)[1])
                  : initials('N', 'A')}
              </div>
            )}
          </div>
      ...
      <div className="w-3.5/10 flex justify-center px-8 py-4 whitespace-nowrap">
        <div className="flex flex-col justify-center items-center cursor-pointer" onClick={handleInstitutionView}>
          <div id={instRowPrps.id} className="w-auto text-sm leading-5 text-gray-900 hover:text-gray-600 ">
      ...
      <div
        className="w-1/10 flex justify-center items-center pr-4 py-4 cursor-pointer whitespace-nowrap text-right text-sm leading-5 font-medium"
        onClick={handleInstitutionView}>
        <div id={instRowPrps.id} className={`flex justify-center ${theme.textColor[themeColor]}`}>
          Edit
        </div>
      </div>
    </div>
