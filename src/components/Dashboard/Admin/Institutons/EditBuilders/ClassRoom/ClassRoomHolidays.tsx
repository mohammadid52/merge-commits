...
const ClassRoomHolidays = ({
  ...
  const onDelete = (index: number) => {
    const onDrop = async () => {
      setDeleting(true);
      const dateToUpdate = lessonImpactLogs.filter(
      ...
      setLessonImpactLogs(
        sortLogsByDate(result?.data?.updateRoom.lessonImpactLog)
      );
      setDeleting(false);
      setLogsChanged(true);
      closeDeleteModal();
