...
const ClassRoomForm = ({ instId }: ClassRoomFormProps) => {
  ...
  const [previousActiveUnitId, setPreviousActiveUnitId] = useState<any | null>(
    null
  );
  ...
  const getTeachersList = async (allInstiId: string[]) => {
    try {
      ...
      if (listStaffs?.length === 0) {
        ...
        setCoTeachersList(
          filteredArray.filter((item: any) => item.id !== teacher.id)
        );
      }
    } catch (err) {
      console.error(err, "err inside catch");
  ...
  const getCurricularList = async (allInstiId: string[]) => {
    try {
      ...
      onStatusUpdate(
        sortedList,
        (roomData?.status || RoomStatus.ACTIVE) as RoomStatus
      );
    } catch (err) {
  ...
  const validateForm = async () => {
    if (roomData.name.trim() === "") {
      ...
      !roomData.curricular.id &&
      roomData.status !== RoomStatus.INACTIVE
    ) {
      setMessages({
        show: true,
  ...
  const saveRoomCurricular = async (
    id: string,
    roomId: string,
    currId: string
  ) => {
    ...
    if (roomId) {
      try {
        ...
        setMessages({
          show: true,
          message:
            RoomBuilderdict[userLanguage]["messages"]["success"][
              "classroomdetail"
            ],
          isError: false,
        });
        ...
        setMessages({
          show: true,
          message:
            RoomBuilderdict[userLanguage]["messages"]["error"]["classroomadd"],
          isError: true,
        });
        setLoading(false);
      }
        ...
        try {
          ...
          if (savedData) {
            ...
            setRoomData({
              ...
              id: savedData.id,
              teachingStyle:
                savedData?.teachingStyle || TeachingStyle.PERFORMER,
              name: savedData.name,
            ...
            <div className="grid grid-cols-2">
              <div className="px-3 py-4">
                <Selector
                  ...
                  placeholder={
                    RoomEDITdict[userLanguage]["INSTITUTION_PLACEHOLDER"]
                  }
              ...
              <div className="px-3 py-4">
                <Selector
                  ...
                  placeholder={
                    RoomEDITdict[userLanguage]["CURRICULUM_PLACEHOLDER"]
                  }
              ...
              <div className="px-3 py-4">
                <Selector
                  ...
                  placeholder={
                    RoomEDITdict[userLanguage]["ACTIVE_UNIT_PLACEHOLDER"]
                  }
            ...
            <div>
              <div className="grid grid-cols-2">
                <div className="px-3 py-4">
                  <SelectorWithAvatar
                    ...
                    placeholder={
                      RoomEDITdict[userLanguage]["TEACHER_PLACEHOLDER"]
                    }
                ...
                <div className="px-3 py-4">
                  <MultipleSelector
                    ...
                    list={coTeachersList}
                    placeholder={
                      RoomEDITdict[userLanguage]["CO_TEACHER_PLACEHOLDER"]
                    }
              ...
              <div className="grid grid-cols-3">
                <div className="px-3 py-4">
                  <FormInput
                    label={
                      RoomEDITdict[userLanguage].CONFERENCE_CALL_LINK_LABEL
                    }
                    name="conferenceCallLink"
                    value={conferenceCallLink}
                    onChange={editInputField}
                    placeHolder={
                      RoomEDITdict[userLanguage]
                        .CONFERENCE_CALL_LINK_PLACEHOLDER
                    }
                  />
                </div>
                <div className="px-3 py-4">
              ...
              {checkIfAdmin() && (
                <CheckBox
                  ...
                  onChange={(e) =>
                    setRoomData({ ...roomData, isZoiq: e.target.checked })
                  }
                  name="isZoiq"
                />
              )}
        ...
        <div className="flex my-8 justify-center">
          ...
          <Buttons
            ...
            label={
              loading
                ? "Saving..."
                : RoomEDITdict[userLanguage]["BUTTON"]["SAVE"]
            }
