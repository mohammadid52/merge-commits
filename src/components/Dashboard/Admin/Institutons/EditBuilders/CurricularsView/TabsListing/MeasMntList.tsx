...
const MeasurementList = (props: MeasurementListProps) => {
  ...
  const onDragEnd = async (result: any) => {
    ...
    if (result.source.index !== result.destination.index) {
      ...
      let measurementList = measurements
        .map((t: any) => {
      ...
      let seqItem: any = await API.graphql(
        graphqlOperation(mutations.updateCSequences, {
          input: {id: `m_${topicID}`, sequence: list}
        })
      );
  ...
  const fetchMeasurements = async () => {
    ...
    let item: any = await API.graphql(
      graphqlOperation(queries.getCSequences, {id: `m_${topicID}`})
    );
    ...
    if (listLength && !sequenceLength) {
      ...
      let seqItem: any = await API.graphql(
        graphqlOperation(mutations.createCSequences, {
          input: {id: `m_${topicID}`, sequence: measurementId}
        })
      );
    ...
    <Fragment>
      {!loading ? (
        <div className="mb-4">
                            ...
                            {(provided, snapshot) => (
                              <div
                                ...
                                <div
                                  ...
                                  <div className="flex w-7/10 px-8 py-3 items-center text-left text-s leading-4 font-medium whitespace-normal text-gray-600 cursor-move">
                                    {' '}
                                    {item.name}{' '}
                                  </div>
                                  <div
                                    className={`flex w-2/10 px-8 py-3 items-center text-left text-s leading-4 cursor-pointer ${theme.textColor[themeColor]}`}
                                    onClick={() => editCurrentMeasurement(item.id)}>
          ...
          <div
            className="w-8/10 mx-auto my-2 flex justify-center items-center px-8 py-4  border-0 border-dashed font-medium border-gray-400 text-gray-600 cursor-pointer"
