      ...
      <div className="w-9/10 mx-auto my-4 flex">
        <div>
          <p className="text-base font-medium text-gray-600">
            {TOPICLISTDICT[userLanguage]['TOPIC']}:{' '}
          </p>
        </div>
      </div>
      {!loading ? (
        <Fragment>
                      ...
                      <div className="w-9/10 m-auto border-b">
                        {topics.map((item: any, index: number) => (
                          <Fragment key={index}>
                            <Draggable key={item.id} draggableId={item.id} index={index}>
                              {(provided, snapshot) => (
                                <div
                                  ...
                                  <div
                                    key={item.id}
                                    className={`flex justify-between w-full px-8 py-4 whitespace-nowrap  border-0  border-b-0 border-gray-200 hover:bg-gray-200 ${
                                      openRow === item.id && 'bg-gray-200'
                                    }`}>
                                    <div
                                      className="flex w-6.5/10 px-8 py-3 items-center text-left text-s leading-4 font-medium whitespace-normal cursor-pointer text-gray-900 hover:text-gray-800"
                                      onClick={() => expandRow(item.id)}>
                                      {' '}
                                      <span className="w-auto mr-4">{index + 1}.</span>
                                      {item.name}{' '}
                                    </div>
                                    <div
                                      className={`flex w-1.5/10 px-8 py-3 text-left text-s leading-4 items-center justify-end cursor-pointer ${theme.textColor[themeColor]}`}
                                      onClick={() => editCurrentTopic(item.id)}>
                                      {TOPICLISTDICT[userLanguage]['EDIT']}
                                    </div>
                                    <div
                                      className="flex w-2/10 items-center px-8 py-3 text-left text-s leading-4 justify-end whitespace-normal"
                                      onClick={() => expandRow(item.id)}>
                                      <span
                            ...
                            {openRow === item.id && (
                              <div className="border-0 border-gray-200">
                                <MeasurementList
          ...
          <div
            className="flex justify-center items-center my-2 w-9/10 mx-auto px-8 py-4  border-0 border-dashed font-medium border-gray-400 text-gray-600 cursor-pointer"
