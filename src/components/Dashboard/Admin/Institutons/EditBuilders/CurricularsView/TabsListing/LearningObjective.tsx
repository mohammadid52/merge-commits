...
const LearningObjectiveList = (props: LearningObjectiveListProps) => {
    ...
    try {
      if (result.source.index !== result.destination.index) {
        ...
        let seqItem: any = await API.graphql(
          graphqlOperation(mutations.updateCSequences, { input: { id: `l_${curricularId}`, sequence: list } })
        );
  ...
  const createLearningObjective = () => {
    history.push(`/dashboard/manage-institutions/${institutionId}/curricular/${curricularId}/learning-objective/add`);
  };
  ...
  const fetchLearningObjs = async () => {
    ...
    let [list, seq]: any = await Promise.all([
      ...
      await API.graphql(graphqlOperation(queries.getCSequences, { id: `l_${curricularId}` })),
    ]);
    ...
    if (listLength && !sequenceLength) {
      ...
      let seqItem: any = await API.graphql(
        graphqlOperation(mutations.createCSequences, { input: { id: `l_${curricularId}`, sequence: learningsID } })
      );
