...
const ProfileCheckpointlookup = () => {
  ...
  const selectItem = (checkpointId: string) => {
    const selectedItem = selectedCheckpointIds.find(
      (id) => id === checkpointId
    );
    let updatedList;
    if (!selectedItem) {
    ...
    try {
      ...
      if (!allCheckpointList) {
        ...
        const sortedList = checkpointList
          .filter(
            (chechpoint: any) => !savedCheckpointId.includes(chechpoint.id)
          )
          .sort((a: any, b: any) =>
        ...
        <Fragment>
          <div className="flex justify-between w-full py-4 whitespace-nowrap border-b-0 border-gray-200">
            <div className="w-1.5/10 px-6 py-3 bg-gray-50 text-center text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
              <span>
                {ProfileCheckpointlookupDict[userLanguage]["selection"]}
              </span>
            </div>
            ...
            {!loading ? (
              <Fragment>
                {!error ? (
                    ...
                    {allCheckpointList?.length ? (
                        ...
                        <Fragment key={item.id}>
                          ...
                          >
                            <div className="flex w-1.5/10 items-center px-8 py-3 text-left text-s leading-4">
                              <span>
                                <CheckBox
                                  value={selectedCheckpointIds?.includes(
                                    item.id
                                  )}
                            ...
                            <div className="flex w-2/10 px-8 py-3 text-left text-s leading-4 items-center whitespace-normal">
                              {item.language
                                ? getLanguageString(item.language)
                                : "--"}
                            </div>
                      ...
                      <div className="py-12 my-6 text-center">
                        <p>
                          {" "}
                          {
                            ProfileCheckpointlookupDict[userLanguage][
                              "listempty"
                            ]
                          }
                        </p>
                  ...
                  <div className="py-12 my-6 text-center">
                    <p>
                      {ProfileCheckpointlookupDict[userLanguage]["errfetch"]}{" "}
                    </p>
                  </div>
                )}
              </Fragment>
      ...
      <div className="flex mt-8 justify-center px-8 pb-4">
        <div className="flex justify-center my-6">
          <Buttons
            ...
            label={
              ProfileCheckpointlookupDict[userLanguage]["button"]["cancel"]
            }
          ...
          {allCheckpointList.length > 0 && (
            <Buttons
              ...
              label={
                loading
                  ? ProfileCheckpointlookupDict[userLanguage]["button"][
                      "saving"
                    ]
              ...
              disabled={
                loading || selectedCheckpointIds.length === 0 ? true : false
              }
            />
          )}
        </div>
