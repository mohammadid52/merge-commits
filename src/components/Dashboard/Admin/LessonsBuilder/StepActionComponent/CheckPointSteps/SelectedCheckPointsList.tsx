...
const SelectedCheckPointsList = (props: SelectedCheckPointsListProps) => {
  ...
  const {SelectedCheckPointsListDict} = useDictionary(clientKey);
      ...
      <div className="px-4 py-5 border-b-0 border-gray-200 sm:px-6 flex items-center">
        ...
        <h4 className="text-base leading-6 font-medium text-gray-900 flex items-center">
          <span className="w-auto flex-shrink-0">
            ...
            {SelectedCheckPointsListDict[userLanguage]['BUILDER']} - {lessonName}
          </span>
        </h4>
      </div>
        ...
        {!activeCheckpoints?.length ? (
          <div className="my-8">
            <p className="text-center p-8">
              {' '}
              {SelectedCheckPointsListDict[userLanguage]['ADDCHECKPOINT']}{' '}
              {lessonType === 'survey' ? 'survey' : 'assessment'} builder
            </p>
            <div className="flex w-full mx-auto p-8 justify-center">
              <Buttons
                btnClass="mr-4"
                onClick={() => changeStep('CheckpointLookup')}
                label={SelectedCheckPointsListDict[userLanguage]['BUTTON']['ADDEXIST']}
              />
              <Buttons
                btnClass="ml-4"
                onClick={() => changeStep('AddNewCheckPoint')}
                label={SelectedCheckPointsListDict[userLanguage]['BUTTON']['CREATE']}
              />
            </div>
          </div>
        ) : (
