...
const Classroom = lazy(
  () => import("components/Dashboard/Classroom/Classroom")
);
const UploadLogsPage = lazy(
  () => import("components/Dashboard/Csv/UploadLogsPage")
);

const GameChangers = lazy(
  () => import("components/Dashboard/GameChangers/GameChangers")
);
const Anthology = lazy(
  () => import("components/Dashboard/Anthology/Anthology")
);
const Profile = lazy(() => import("components/Dashboard/Profile/Profile"));
const TestCases = lazy(
  () => import("components/Dashboard/TestCases/TestCases")
);
  ...
  let getSyllabusInSequence =
    sequenceArray && sequenceArray.length > 0
      ? sequenceArray?.reduce((acc: any[], syllabusID: string) => {
          return [
            ...acc,
            filteredSyllabusArray.find(
              (syllabus: any) => syllabus.unitId === syllabusID
            ),
          ];
        }, [])
                ...
                items:
                  syllabus?.unit.lessons?.items?.length > 0
                    ? syllabus?.unit.lessons.items
                        .map((t: any) => {
                          let index = syllabus?.universalLessonsSeq?.indexOf(
                            t.id
                          );
                          return { ...t, index };
                        })
...
const Dashboard = () => {
  ...
  async function getUser() {
    ...
    const userAuthId = stateUser?.authId
      ? stateUser?.authId
      : cookies.auth?.authId;
    try {
      const queryObj = {
      ...
      logError(
        error,
        { authId: userAuthId, email: userEmail },
        "Dashboard @getUser"
      );
  ...
  useEffect(() => {
    ...
    const getRoomFromState = roomData.rooms.find(
      (room: any) => room.id === activeRoom
    );
  ...
  const handleRoomSelection = (
    ...
    const getRoomSyllabus = roomData.rooms.find(
  ...
  return (
    <>
      <div
        id="top-menu"
        className={`w-full ${isGameChangers ? "bg-black" : "bg-white"}`}
      >
