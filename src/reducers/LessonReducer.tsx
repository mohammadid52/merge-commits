...
export const lessonReducer = (state: any, action: LessonActions) => {
  switch (action.type) {
    ...
    case LESSON_REDUCER_TYPES.SET_ROOM_SUBSCRIPTION_DATA:
      const havePagesChanged = Object.keys(action.payload).includes(
        "ClosedPages"
      );
    ...
    case LESSON_REDUCER_TYPES.ADD_NEW_INPUT:
      ...
      const currentPageStudentData = [
        ...oldStudentData[state.currentPage],
        _newInput,
      ];
    ...
    case LESSON_REDUCER_TYPES.UPDATE_STUDENT_DATA:
      ...
      const updatedTargetStudentData =
        state?.studentData[pageIdx].map(
          (studentPageInput: StudentPageInput) => {
            return {
              domID: studentPageInput.domID,
              input:
                studentPageInput.domID === domID
                  ? newInput
                  : studentPageInput.input,
            };
          }
        ) || [];
    ...
    case LESSON_REDUCER_TYPES.UPDATE_SURVEY_DATA:
      ...
      return {
        ...
        updated: true,
        universalStudentDataID: [
          { ...state.universalStudentDataID[0], update: true },
        ],
    ...
    case LESSON_REDUCER_TYPES.SET_CURRENT_PAGE:
      return {
        ...
        lessonProgress:
          action.payload > state.lessonProgress
            ? action.payload
